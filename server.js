...
const oddNumberBackupEmails = ["TODO", "TODO2"]; // TODO: ask facilitators if they want in
...
const getSubscribedEmails = async ({ zulipAPI, users }) => {
  ...
  const allSubscribedEmails = botSubs.filter(sub => sub.stream_id === 142655)[0].subscribers;
  return allSubscribedEmails.filter(email => {
    return email !== zulipConfig.username && !(getUserWithEmail({ users, email }).is_bot)
  });
}
...
const tryToGetUsernameWithEmail = ({ users, email }) => {
  try {
    // console.log('tryUsername',getUserWithEmail({ users, email }))
...
const matchEmails = async ({ emails }) => {
  ...
  while (unmatchedEmails.length > 0) {
    ...
    const availableEmails = unmatchedEmails.filter(email => !matchedEmails.includes(email))
    // console.log(">>>>> \n", currentEmail, "\n", matchedEmails, "\n", availableEmails)
    ...
    } else { // this should only happen on an odd number of emails
       // TODO: how to handle the odd person
      newMatches.push([currentEmail, oddNumberBackupEmails[Math.floor(Math.random() * oddNumberBackupEmails.length)]])
    }
...
const sendAllMessages = ({ zulipAPI, matchedEmails, users }) => {
  matchedEmails.forEach((match) => {
    ...
    db.run(`INSERT INTO matches(date, email1, email2) VALUES ("${(new Date).toISOString().split("T")[0]}", "${sortedMatch[0]}", "${sortedMatch[1]}")`)
    sendMessage({ zulipAPI, toEmail: match[0], matchedName: tryToGetUsernameWithEmail({ users, email: match[1] }) })
    sendMessage({ zulipAPI, toEmail: match[1], matchedName: tryToGetUsernameWithEmail({ users, email: match[0] }) })
  })
}
...
const run = async () => {
  ...
  // const activeEmails = ["a", "b", "c", "d", "e", "f", "g"]
